...
def update_resume(request):
    if request.user.is_applicant:
        resume, created = Resume.objects.get_or_create(user=request.user)  # Ensures a Resume object exists
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                messages.warning(request, 'Something went wrong')  # Fixed missing request parameter
        else:
